...
mkdir -p outputs/triposr hf_cache
docker run --gpus all --rm \
  ...
  "$IMG" bash -lc '
    ...
    python run.py /work/inputs/harp_front_white.png \
      --output-dir /work/outputs/triposr \
      --mc-resolution 256 --model-save-format obj 2>&1 | tail -20
    echo "== outputs =="
    ls -laR /work/outputs/triposr
  '
